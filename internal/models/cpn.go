...
type CPN struct {
	...
	Arcs           []*Arc            `json:"arcs"`
	InitialMarking map[string][]*Token `json:"initialMarking"` // Initial tokens by place name
	EndPlaces      []string          `json:"endPlaces"`       // Places that signify case completion
}
...
func (cpn *CPN) SetInitialMarking(placeName string, tokens []*Token) {
	cpn.InitialMarking[placeName] = tokens
}

// AddInitialToken adds a token to the initial marking of a place
func (cpn *CPN) AddInitialToken(placeName string, token *Token) {
	cpn.InitialMarking[placeName] = append(cpn.InitialMarking[placeName], token)
}
...
func (cpn *CPN) CreateInitialMarking() *Marking {
	...
	for placeName, tokens := range cpn.InitialMarking {
		for _, token := range tokens {
			marking.AddToken(placeName, token.Clone())
		}
	}
...
func (cpn *CPN) ValidateStructure() []error {
	...
	for placeName := range cpn.InitialMarking {
		if cpn.GetPlaceByName(placeName) == nil {
			errors = append(errors, fmt.Errorf("initial marking references non-existent place: %s", placeName))
		}
	}
	...
	for _, endPlace := range cpn.EndPlaces {
		if cpn.GetPlaceByName(endPlace) == nil {
			errors = append(errors, fmt.Errorf("end place references non-existent place: %s", endPlace))
		}
	}
...
func (cpn *CPN) Clone() *CPN {
	...
	for placeName, tokens := range cpn.InitialMarking {
		...
		clone.InitialMarking[placeName] = clonedTokens
	}
